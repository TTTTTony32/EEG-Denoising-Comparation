...
def run():
    ...
    train_x = np.load("./datasets/train_input.npy")
    train_y = np.load("./datasets/train_output.npy")
    test_x = np.load("./datasets/test_input.npy")
    test_y = np.load("./datasets/test_output.npy")
    your_data = {
    'train_x': train_x,    # 训练集输入矩阵 (N_train, 512)
    'train_y': train_y,   # 训练集输出矩阵 (N_train, 512)
    'test_x': test_x,      # 测试集输入矩阵 (N_test, 512)
    'test_y': test_y,     # 测试集输出矩阵 (N_test, 512)
    }
    train_dataset, test_dataset = load_dataset(config, custom_data=your_data)
