...
from EEGIFNet import MA_INet, MA_MNet
...
def pplot(I,M,test_input,test_output):
    ...
    for i in range(num):
        ...
        plt.title('(3)',y=-0.3)
        #plt.legend([l0, l1, l2], ['Raw EEG', 'Denoised EEG', 'Clean EEG'], loc='upper right')
        plt.savefig('EEGIFNet/checkpoint/EMG_'+str(i)+'.png',dpi = 300)
        plt.close()
...
if __name__ == '__main__':
    ...
    # sample_output = torch.tensor(test_output[index:index+1]).float().to(device)

    # I.eval()
    # M.eval()
    # with torch.no_grad():
    #     predict_eeg, predict_a = I(sample_input)
    #     outputs = M(sample_input, predict_eeg, predict_a)

    # l0, = plt.plot(sample_input.cpu().squeeze(), label='Raw EEG')
    # l1, = plt.plot(outputs.cpu().squeeze(), label='Denoised EEG')
    # l2, = plt.plot(sample_output.cpu().squeeze(), label='Clean EEG')
    # plt.legend([l0, l1, l2], ['Raw EEG', 'Denoised EEG', 'Clean EEG'], loc='upper right')
    # plt.show()
    # plt.savefig('EEGIFNet/checkpoint/1.png', dpi=300)
